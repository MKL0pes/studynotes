...
export const Route = createFileRoute("/login")({
  validateSearch: (s: Record<string, unknown>) => ({
    redirect: typeof s.redirect === "string" ? s.redirect : "/_authenticated/dashboard",
  }),
  head: () => ({
    meta: [
...
function LoginPage() {
  ...
  if (session) return <Navigate to={search.redirect} replace />;
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-background via-background to-accent/30 px-4 py-12">
      <div className="w-full max-w-md">
        ...
        <div className="rounded-2xl border border-border bg-card p-6 shadow-soft">
          ...
          {mode === "forgot" && (
            <button type="button" onClick={() => setMode("login")} className="mt-4 w-full text-center text-xs text-muted-foreground hover:text-foreground">
              ← Voltar ao login
            </button>
          )}
        </div>
        ...
        <p className="mt-6 text-center text-xs text-muted-foreground">
          <Link to="/" className="hover:text-foreground">StudyNotes © 2026</Link>
        </p>
      </div>
    </div>
  );
